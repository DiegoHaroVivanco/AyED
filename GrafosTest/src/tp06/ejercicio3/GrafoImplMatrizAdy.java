...
public class GrafoImplMatrizAdy<T>
    ...
    public void conectarBidireccional(Vertice<T> origen, Vertice<T> destino) {
    	conectar(origen, destino, 1);
    	conectar(destino, origen, 1);
    	
    }
    
    @Override
    public void conectarBidireccional(Vertice<T> origen, Vertice<T> destino, int peso) {
    	matrizAdy[((VerticeImplMatrizAdy<T>) origen).getPosicion()][((VerticeImplMatrizAdy<T>) destino) .getPosicion()] = peso;
    	matrizAdy[((VerticeImplMatrizAdy<T>) destino).getPosicion()][((VerticeImplMatrizAdy<T>) origen) .getPosicion()] = peso;
    }
    ...
    public void desConectarBidireccional(Vertice<T> origen, Vertice<T> destino) {
    	matrizAdy[((VerticeImplMatrizAdy<T>) origen).getPosicion()][((VerticeImplMatrizAdy<T>) destino).getPosicion()] = 0;
    	matrizAdy[((VerticeImplMatrizAdy<T>) destino).getPosicion()][((VerticeImplMatrizAdy<T>) origen).getPosicion()] = 0;
    }
